...
df_people = spark.read.option('header', 'true').schema(people_schema).csv('./spark_demo_course/3_Spark_Basics/data/top_100_richest.csv')

df_country = spark.read.option('header', 'true').schema(country_schema).csv('./spark_demo_course/3_Spark_Basics/data/wiki_number_of_billionaires.csv')
...
df_parquet = spark.read.schema(people_schema).parquet('./spark_demo_course/3_Spark_Basics/data_parquet')
...
with open('./spark_demo_course/3_Spark_Basics/for_history/plans.txt', 'w') as file:
    file.write(plans)
    
#я оставил оба варианта вывода планов, но второй который plans_1 предпочтительнее. Планы одинаковые

with open('./spark_demo_course/3_Spark_Basics/for_history/plans1.txt', 'w') as file:
    file.write(plans_1)
